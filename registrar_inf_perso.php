<?php
// =======================================================
// Archivo: registrar_inf_perso.php
// Autor: Sistema Escolar
// Fecha de actualización PHP: 15-10-2025 (PHP 8.2)
// Descripción: Informe de Personalidad 1er Semestre
// =======================================================
...
switch ($btnaccion) {
    case "Consultar":
        ...
        if ($num == 1) $consulta_perso = "SELECT * FROM perso$periodo WHERE grado='$grado' AND periodo='$periodo' AND semestre='1' AND cod_ambito<='11' ORDER BY id_perso";
        if ($num == 2) $consulta_perso = "SELECT * FROM perso$periodo WHERE grado='$grado' AND periodo='$periodo' AND semestre='1' AND cod_ambito>=12 AND cod_ambito<=20";
        if ($num == 3) $consulta_perso = "SELECT * FROM perso$periodo WHERE grado='$grado' AND periodo='$periodo' AND semestre='1' AND cod_ambito>=21";
        ...
        if ($total_perso <= 0) {
            ...
            echo "<script>alert('Este Curso ya tiene registrado el informe de personalidad en este número de Ámbito del primer semestre, los datos guardados se mostrarán, si desea, puede modificarlos y luego oprima el botón Actualizar para guardar los cambios...!!!');</script>";
            $t = 0;
            $cod_ambitop = [];
            ...
            for ($t = 0; $t < $total_ambito; $t++) {
                for ($i = 0; $i < $total_consultar; $i++) {
                    $lit[$t][$i] = "";
                    $consulta_perso2 = "SELECT * FROM perso$periodo WHERE grado='$grado' AND periodo='$periodo' AND semestre='1' AND cod_ambito='{$cod_ambitop[$t]}' AND ci_alu='{$ci_alu[$i]}' AND ci_alu2='{$ci_alup[$i]}'";
    ...
    case "Guardar":
        ...
        $desc_reg = 'Actualizó';
        $registro = 'Procesos, Personalidad 1er Semestre';
...
<div class="caja">
    <font class="sombra" color="#00366C" face="Arial Black, Gadget, sans-serif" size="5">
        INFORME DE PERSONALIDAD 1ER SEMESTRE
    </font>
