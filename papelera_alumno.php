...
if ($btnaccion == 'Recuperar' && $total_sql > 0) {
    for ($i = 1; $i <= $total_sql; $i++) {
        ...
        $txtci_promover_array[$i] = isset($_POST["txtci_promover".$i]) ? $_POST["txtci_promover".$i] : '';
        ...
        if ($total_sql > 0) {
            for ($i = 1; $i <= $total_sql; $i++) {
                if (isset($chkreprovar[$i]) && $chkreprovar[$i] == "SI") {
                    $conexion = conectarse_escuela();
                    $ci_alu2 = mysqli_real_escape_string($conexion, $txtci_promover_array[$i]);
                    $ci_alu = mysqli_real_escape_string($conexion, $txtced_promover_array[$i]);
                    
                    // Actualizar el registro del alumno
                    $consulta = "UPDATE alumno SET borrado = 0 
                               WHERE ci_alu = ? AND ci_alu2 = ?";
                    
                    if ($stmt = mysqli_prepare($conexion, $consulta)) {
                        mysqli_stmt_bind_param($stmt, "ss", $ci_alu, $ci_alu2);
...
<body>
    <div class="container">
        <form id="form1" name="form1" method="post" action="papelera_alumno.php" class="form-container">
            <div class="caja">
                <div class="titulo-principal sombra">
                    PAPELERA DE RECICLAJE DEL ALUMNO
                </div>
            ...
            <div>
                <?php
                $consulta_sql = "SELECT * FROM alumno WHERE borrado = 1";
                    ...
                    ?>
                        <tr style="background-color: <?php echo $color; ?>">
                            ...
                            <td style="text-align: right;">
                                <?php echo htmlspecialchars($datos_sql['ci_alu'] . '-' . $datos_sql['ci_alu2']); ?>
                                <input name="txtci_promover<?php echo $i; ?>" type="hidden" 
                                       value="<?php echo htmlspecialchars($datos_sql['ci_alu2']); ?>">
                                <input name="txtced_promover<?php echo $i; ?>" type="hidden" 
                                       value="<?php echo htmlspecialchars($datos_sql['ci_alu']); ?>">
                            </td>
                            <td style="text-align: left;">
                                <?php echo htmlspecialchars($datos_sql['nom_alu']); ?>
                            </td>
