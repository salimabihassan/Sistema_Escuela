<?php
/**
 * MÓDULO: Registro de Notas del Primer Semestre
 * 
 * Propósito:
 *   Permite a los docentes consultar, ingresar y actualizar las notas del primer semestre
...
// Establecer codificación de salida como UTF-8 (importante para tildes y ñ)
header('Content-Type: text/html; charset=UTF-8');

// Mostrar errores (solo en desarrollo; en producción, desactivar)
...
// Validar que la sesión contenga las variables mínimas requeridas
...
/**
 * Función auxiliar para escapar cadenas y prevenir inyecciones SQL
 * @param string $str Cadena a escapar
 * @param mysqli $conn Conexión mysqli activa
 * @return string Cadena segura para usar en consultas SQL
 */
...
// Procesar datos POST si el formulario fue enviado
...
// Fecha actual en formato d/m/Y (solo para mostrar en pantalla)
$fecha_insc = date("d/m/Y");

// Inicializar arrays para almacenar notas y promedios
$nota = [];
$id_notas = [];
$prom = [];
$txtci_alu = [];
...
if ($btnaccion === 'Calcular') {
    if (empty($asig)) {
        ...
        $y = 0; // Bandera para mostrar alerta solo una vez
        for ($t = 1; $t <= $total_alu; $t++) {
            $p = 0; // Contador de notas válidas
            $prom[$t] = 0;
            for ($i = 0; $i < 10; $i++) {
                $nota_val = floatval($_POST["nota{$t}{$i}"] ?? 0);
                $id_notas[$t][$i] = $_POST["id_notas{$t}{$i}"] ?? '';

                // Validar rango de notas (1 a 7, o 10 a 70 que se convierten)
                if ($nota_val >= 1 && $nota_val < 71) {
                    if ($nota_val > 7) $nota_val /= 10; // Convertir nota de 10-70 a 1-7
                    $p++;
                    $prom[$t] += $nota_val;
                } elseif ($nota_val >= 1) {
                    // Nota fuera de rango
                ...
                // Formatear nota para mostrar (1 decimal) o dejar vacío si es 0
                $nota[$t][$i] = ($nota_val < 1) ? "" : number_format($nota_val, 1, '.', '');
            }

            // Calcular promedio si hay notas válidas
            if ($p > 0) {
            ...
            // Si es asignatura especial (cod_asig == 100), convertir a letras
...
// ACCIONES PRINCIPALES: Consultar, Guardar
switch ($btnaccion) {
    case "Consultar":
        ...
        // Verificar si ya existen notas para este curso y asignatura
        $cod_curso = $grado . $periodo;
        $consulta_nota = "SELECT * FROM notas{$periodo} WHERE cod_curso = '$cod_curso' AND cod_asig = '$cod_asig' AND semestre = '1'";
        $resultado_nota = mysqli_query($conexion, $consulta_nota);
        $total_nota = mysqli_num_rows($resultado_nota);

        if ($total_nota <= 0) {
            // No hay notas: inicializar tabla vacía
            for ($t = 1; $t <= $total_alu; $t++) {
                for ($i = 0; $i < 10; $i++) {
                    $nota[$t][$i] = "";
                }
            ...
            // Ya existen notas: cargarlas desde la base de datos
            ...
            while ($datos_alu2 = mysqli_fetch_array($resultado_alu2)) {
                ...
                $consulta_nota2 = "SELECT * FROM notas{$periodo} WHERE cod_curso = '$cod_curso' AND cod_asig = '$cod_asig' AND semestre = '1' AND ci_alu = '$ci_a' AND ci_alu2 = '$ci_b' ORDER BY id_notas";
                ...
                // Rellenar con campos vacíos si hay menos de 10 notas
                while ($i < 10) {
                    $nota[$t][$i] = "";
                    $id_notas[$t][$i] = "";
                    $i++;
    ...
    case "Guardar":
        ...
        // Datos del usuario que realiza la acción (para auditoría)
        $ci_usu = esc($_SESSION["cedula"], $conexion);
        $ci_usu2 = esc($_SESSION["ci_usu2"], $conexion);
        $FechaMySQL = date("Y-m-d"); // Fecha en formato MySQL

        $x = 0; // Bandera para alerta de modificación bloqueada
        for ($t = 1; $t <= $total_alu; $t++) {
            $prom_t = $_POST["prom{$t}"] ?? '';
            ...
            for ($i = 0; $i < 10; $i++) {
                ...
                if (!empty($id_nota)) {
                    ...
                    if ($res2 && mysqli_num_rows($res2) > 0) {
                        ...
                        if ($row['final'] == 0) {
                            // Actualizar si no está bloqueado
                            ...
                            // Registro bloqueado (informe ya impreso)
                            if ($x < 1) {
                    ...
                    // Insertar nuevo registro (solo si la nota es válida)
                    if ($nota_val >= 1) {
                        $insert = "INSERT INTO notas{$periodo} (ci_alu, ci_alu2, cod_asig, cod_curso, periodo, semestre, nota_prom, nota, fecha) 
                                   VALUES ('$txtci_alu_t', '$txtci_alua_t', '$cod_asig', '$cod_curso', '$periodo', '1', $promedio_usar, $nota_formateada, '$FechaMySQL')";
...
// Consultar alumnos para mostrar en la tabla (solo si se seleccionaron grado, periodo y asignatura)
$consulta_alu = "";
$total_alu = 0;
...
<script>
/**
 * Función para permitir solo números y punto decimal en campos de nota
 */
  ...
  <tr align="left">
    ...
    <td width="63" bgcolor="#F9F9F9">
      <select name="grado" size="1" onChange="submit();" id="grado">
        <option value=""></option>
        <?php
        // Mostrar solo los grados permitidos según el nivel del usuario
        $grados = [];
        ...
        foreach ($grados as $g) {
            ...
            // Generar sufijo: 1ro, 2do, 3ro, 4to, etc.
    ...
    <td width="148" bgcolor="#F9F9F9">
      <?php
      // Consultar asignaturas según el grado seleccionado
      $consulta_grado = "";
...
<!-- Tabla de alumnos y notas (solo se muestra si hay datos) -->
