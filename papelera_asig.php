        ...
        if ($total_sql > 0) {
            for ($i = 1; $i <= $total_sql; $i++) {
                if (isset($chkreprovar[$i]) && $chkreprovar[$i] == "SI") {
                    $conexion = conectarse_escuela();
                    $cod = mysqli_real_escape_string($conexion, $cod_promover_array[$i]);
                    
                    // Actualizar el registro de la asignatura
                    $consulta = "UPDATE asignatura SET borrado = 0 
                               WHERE cod_asig = ?";
...
<head>
    ...
    <title>PAPELERA DE RECICLAJE - ASIGNATURA</title>
    <style>
        /* Reset básico */
        body, td, th {
...
<body>
    <div class="container">
        <form id="form1" name="form1" method="post" action="papelera_asig.php" class="form-container">
            <div class="caja">
                <div class="titulo-principal sombra">
                    PAPELERA DE RECICLAJE DE ASIGNATURA
                </div>
            ...
            <div>
                <?php
                $consulta_sql = "SELECT * FROM asignatura WHERE borrado = 1";
                    ...
                    ?>
                        <tr style="background-color: <?php echo $color; ?>">
                            ...
                            <td style="text-align: center;">
                                <?php echo htmlspecialchars($datos_sql['cod_asig']); ?>
                                <input name="cod_promover<?php echo $i; ?>" type="hidden" 
                                       value="<?php echo htmlspecialchars($datos_sql['cod_asig']); ?>">
                            </td>
                            <td style="text-align: left;">
                                <?php echo htmlspecialchars($datos_sql['nom_asig']); ?>
                            </td>
                            <td style="text-align: center;">
